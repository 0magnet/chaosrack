...
func runDemo() {
	...
	rng := rand.New(rand.NewSource(*demoSeed)) //nolint:gosec // picking demo values and fake input; not a security decision
	...
	if *demoFrames != "" {
		_ = os.MkdirAll(*demoFrames, 0o750) //nolint:errcheck
	}
	act := ""
	rngs := []*rand.Rand{rng}
	for i := 1; i < *demoMonkeys; i++ {
		rngs = append(rngs, rand.New(rand.NewSource(*demoSeed+int64(i)*7919))) //nolint:gosec // picking demo values and fake input; not a security decision
	}
	deadline := time.Time{}
	if *demoDur > 0 {
