...
func niam() {
	...
	// Detached BEFORE it is freed. Releasing a callback that is still listening
	// leaves the button wired to something that no longer exists, and clicking
	// it then panics -- and this particular button is the one a person presses
	// again when the first press appears to do nothing.
	defer func() {
		sB.Call("removeEventListener", "click", sBc)
		sBc.Release()
	}()
