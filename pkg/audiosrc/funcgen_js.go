...
// Drain fills dst with freshly synthesized samples and ALWAYS RETURNS A FULL
// BUFFER, because a generator has no backlog: there is no amount of reading
// that exhausts it.
//
// Callers must therefore bound their own loops. A drain loop written as "keep
// going until a partial fill" is correct for a ring buffer fed by a device and
// is an infinite loop here — one such loop in the spectrogram allocated until
// the wasm heap died, which is a blank page rather than a broken feature.
