...
func NewMic(opts MicOptions) Source {
	...
	// These two are NOT released, and that is the careful choice rather than an
	// oversight. Exactly one of them ever fires, so neither can free itself the
	// way a one-shot normally does; and freeing them in Close would be worse,
	// because the permission prompt can sit open for as long as the person
	// takes to answer it. A callback released while its promise is still
	// pending is called after release when the answer arrives, and calling a
	// released js.Func panics — which in wasm is a blank page rather than a
	// missing microphone.
	//
	// The cost is two callbacks per mic activation, which is bounded by how
	// many times somebody toggles the switch. onStream already refuses to do
	// anything if the source was closed while the prompt was up, and stops the
	// tracks it was handed, so a late grant leaves nothing running.
...
func (m *micSource) pullChannel(inBuf js.Value, ch int, r *ring) {
	...
	samples := unsafe.Slice((*float32)(unsafe.Pointer(&m.byteScratch[0])), n) //nolint:gosec // reinterpreting a typed slice as its backing bytes to cross into JS without a second copy
	r.write(samples)
}
