...
// usesVar reports whether the compiled expression reads a state-variable slot
// (the numbering is varIndex's: 0=x 1=y 2=z 3=w 4=t).
//
// The iterate flavor asks this before it will run a system as a map. Reading
// the RPN is the only honest way to know: the parser folds w and t into the
// same token kind as x, so nothing downstream can tell that an expression
// depends on a quantity a map does not have.
func (e *Expr) usesVar(slot int) bool {
	if e == nil {
		return false
	}
	for _, tk := range e.rpn {
		if tk.kind == tkVar && tk.vi == slot {
			return true
		}
	}
	return false
