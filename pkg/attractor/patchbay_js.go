...
func patchBank() []string {
	...
	raw := ls.Call("getItem", patchStoreKey)
	if !raw.Truthy() {
		return bank
	}
...
func patchBankStore(bank []string) {
	if ls := js.Global().Get("localStorage"); ls.Truthy() {
		ls.Call("setItem", patchStoreKey, strings.Join(bank, "\x1f"))
	}
}

// patchRecall resets to defaults, then re-applies the snapshot.
func patchRecall(snapshot string) {
	if snapshot == "" {
		return
	}
...
// hashModeOf extracts the mode token from a serialized snapshot.
