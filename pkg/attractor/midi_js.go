...
func startMIDI() {
	...
	// Denial is a normal answer, and it arrives as a REJECTED PROMISE. Without
	// this the refusal went nowhere: the browser logged an unhandled rejection
	// that nobody reads, the switch stayed on, and MIDI simply never worked —
	// indistinguishable, from the person's side, from having no device plugged
	...
	nav.Call("requestMIDIAccess").Call("then", then, fail)
}
