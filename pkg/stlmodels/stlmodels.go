// Package stlmodels is the built-in solid models: the rack as a thing you
// could hold, the geometry as closed solids rather than wireframes, and every
// registered flow swept as a tube along its own trajectory.
//
// Not in meshstl, which builds meshes and knows nothing of the app: these are
// the app's rack and the app's flows. The same list backs `cmd/stlgen` and the
// STL mode's model picker, so what is written to a file and what the app
// offers cannot be different sets.
package stlmodels
...
func (m Model) Build(seg int) meshstl.Mesh { return m.build(seg) }
...
// fetching anything.
func (m Model) Bytes(seg int) ([]byte, error) {
...
//
// ViewerSeg exists to keep a swept attractor tube inside the viewer's
...
// All returns every built-in, rack first, then geometry, then one per
// registered flow in catalog order. name gives a flow's display name from its
// key — the app's mode picker owns those — and nil leaves flows under their
// keys.
func All(name func(key string) string) []Model {
	out := []Model{
		{
...
// MaxTris is the viewer's triangle budget: three vertices per
...
// ViewerSeg is the segment count to build a model at for the app's own STL
// mode rather than for a file.
//
// It matters because the viewer indexes with 16 bits and decimates anything
// over MaxTris by dropping every Nth TRIANGLE — which on a swept tube
