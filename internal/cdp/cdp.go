...
func Dial(port int, urlSubstr string) (*Client, error) {
	...
	for _, t := range tabs {
		if t.Type == "page" && strings.Contains(t.URL, urlSubstr) && t.WebSocketDebuggerURL != "" {
			...
			_, _ = c.Call("Runtime.enable", nil)
			_, _ = c.Call("Page.enable", nil)
			_, _ = c.Call("Page.bringToFront", nil) // avoid rAF/timer throttling while occluded
			return c, nil
		}
	}
...
func (c *Client) Call(method string, params map[string]any) (map[string]any, error) {
	c.id++
	myID := c.id
	req, _ := json.Marshal(map[string]any{"id": myID, "method": method, "params": params})
...
func (c *Client) EvalJSON(expr string) map[string]any {
	...
	var m map[string]any
	if v != "" {
		_ = json.Unmarshal([]byte(v), &m) // best-effort: callers treat missing keys as absent
	}
	return m
}
...
func (c *Client) Mouse(kind string, x, y float64, buttons int) {
	...
	_, _ = c.Call("Input.dispatchMouseEvent", p)
}
...
func (c *Client) Wheel(x, y, dy float64) {
	_, _ = c.Call("Input.dispatchMouseEvent", map[string]any{"type": "mouseWheel", "x": x, "y": y, "deltaX": 0, "deltaY": dy})
}
